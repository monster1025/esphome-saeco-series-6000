#include <cstdint>
#include <vector>
#include <cstdio>
#include <cassert>

static const uint8_t BitReverse[256] = {
  
};

uint32_t calc_crc(const std::vector<uint8_t>& data, size_t start, size_t end) {
    uint32_t crc = 0xFFFFFFFF;
    const uint32_t POLY = 0x04C11DB7;
    if (start >= end) return 0;
    uint8_t val = data[start];
    ((uint8_t*)(&crc))[3] ^= BitReverse[val];
    for (uint8_t j = 0; j < 8; j++) {
        if (crc & 0x80000000) {
            crc = (crc << 1) ^ POLY;
        } else {
            crc <<= 1;
        }
    }
    for (size_t i = start + 1; i < end; i++) {
        uint8_t v = data[i];
        ((uint8_t*)(&crc))[3] ^= BitReverse[v];
        for (uint8_t j = 0; j < 8; j++) {
            if (crc & 0x80000000) {
                crc = (crc << 1) ^ POLY;
            } else {
                crc <<= 1;
            }
        }
    }
    uint32_t t32 = 0;
    ((uint8_t*)(&t32))[0] = BitReverse[((uint8_t*)(&crc))[3]];
    ((uint8_t*)(&t32))[1] = BitReverse[((uint8_t*)(&crc))[2]];
    ((uint8_t*)(&t32))[2] = BitReverse[((uint8_t*)(&crc))[1]];
    ((uint8_t*)(&t32))[3] = BitReverse[((uint8_t*)(&crc))[0]];
    crc = t32 ^ 0xFFFFFFFF;
    return crc;
}

int main() {
    // Пример команды (без CRC и 0x55):
    std::vector<uint8_t> pkt = {
        0xAA,0xAA,0xAA,0x90,0x0B,0x0A,0x05,0x03,0x00,0x02,0x00,0x00,0x64,0x00,0x00,0x00,0x39,0xBF,0x5B,0x74,0x55
    };
    size_t preamble = 3;
    size_t crc_start = preamble;
    size_t crc_end = pkt.size() - 5;
    uint32_t crc = calc_crc(pkt, crc_start, crc_end);
    printf("CRC for test packet: %08X\n", crc);
    // assert(crc == 0x????????); // подставьте ожидаемое значение CRC
    return 0;
} 